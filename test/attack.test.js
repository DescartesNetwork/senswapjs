...
function attackerInfo(data) {
  ...
  console.log(`*** Value: $${div(data.a, DECIMALS) * 100000 + div(data.b, DECIMALS) * 0.1}`);
  console.groupEnd();
}

class MarketShockResistance {
  constructor(p0) {
    this.p = p0;
    this.mu = 0;
    this.discount = 0.9;
  ...
  next = (p) => {
    this.mu = Math.abs(p - this.p) / this.p + this.discount * this.mu;
    this.p = p;
    console.group();
    console.log('Micro', this.mu);
...
describe('Attack scenario', function () {
  it('Attack based on slippage', function (done) {
    ...
    const msr = new MarketShockResistance(price(B));
    msr.next(price(B));
    console.log('\n');

    // Swap A for B
    const a = 1n * DECIMALS / 10n ** 4n;
