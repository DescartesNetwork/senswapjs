...
describe('Swap library', function () {
  ...
  describe('Test Pool', function () {
    ...
    it('Should be a successful swap', function (done) {
      ...
      swap.getPoolData(POOL_ADDRESS_0).then(data => {
        ...
        return swap._splt.getAccountData(VAULT_ADDRESS_0);
      }).then(data => {
        return done();
      }).catch(er => {
        return done(er);
  ...
  describe('Test LPT', function () {
    ...
    it('Should add liquidity', function (done) {
      ...
      swap.initializeLPT(lpt, MINT_LPT_ADDRESS_0, payer).then(txId => {
    ...
    it('Should close LPT Account', function (done) {
      const swap = new Swap();
      const mintLPT = createAccount();
      const vault = createAccount();
      const treasuryS = createAccount();
      ...
      payer.getAccount().then(re => {
        ...
        return swap.closeLPT(lptAddress, payer);
      }).then(txId => {
        return done();
      }).catch(er => {
        return done(er);
